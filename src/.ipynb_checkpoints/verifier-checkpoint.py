...
class VerifierThresholds:
    ...
    # Oscillation
    osc_pass: float = 0.20        # fraction of sign changes in Δx – pass
    osc_fail: float = 0.50        # above this → oscillatory (under-reg)
...
def _oscillation_score(
    ...
    """Fraction of consecutive pairs in x that change sign."""
    if len(x) < 2:
        return 0.0
    dx = np.diff(x)
    sign_changes = int(np.sum(np.diff(np.sign(dx)) != 0))
    osc = sign_changes / max(len(dx) - 1, 1)
